...
Spinupmail dispatches integration events asynchronously through a queue. Create
the queue used for integration dispatch jobs:

```bash
pnpm exec wrangler queues create spinupmail-integration-dispatches
```

See [Integrations](/docs/integrations) for provider setup and subscription
flow.
...
- `FORCED_MAIL_PREFIX = temp` forces all created and renamed inbox local parts
  to start with `temp-`
